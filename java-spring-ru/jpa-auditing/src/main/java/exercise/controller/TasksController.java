...
public class TasksController {
    ...
    public Task update(@PathVariable long id, @RequestBody Task taskBody) {
        var tsk = taskRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Task " + id + " not found"));

        tsk.setTitle(taskBody.getTitle());
        tsk.setDescription(taskBody.getDescription());
